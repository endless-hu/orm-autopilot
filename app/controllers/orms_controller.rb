...
class OrmsController < ApplicationController
  ...
  def create
    ...
    flash[:notice] = 'Chat was successfully created. Please check back in about 1 minute for feedback.'
  ...
  def update
    ...
    GenerateFeedbackJob.perform_later(@chat.id)
    flash[:notice] = 'Chat was successfully updated. Please check back in about 1 minute for feedback.'
    redirect_to user_orm_path(@current_user, @chat)
  end
